...
if (isset($_POST['reg_user'])) {
  ...
  $user_check_query = "SELECT * FROM signin WHERE username='$username' OR email='$email' LIMIT 1";
  $result = mysqli_query($db, $user_check_query);
  $signin = mysqli_fetch_assoc($result);
  
  if ($signin) { // if user exists
  ...
  if (count($errors) == 0) {
  	...
  	$query = "INSERT INTO signin (username,email,password) 
